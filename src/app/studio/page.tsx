...
- All LaTeX MUST be wrapped in $...$ (inline) or $$...$$ (display). Never write bare LaTeX commands like 
rac, \sin, 
ec outside math delimiters.
- Use 
rac{a}{b} for fractions. Never write a/b for math fractions or use frac without backslash.
- Use 
ec{a} for vectors, \hat{a} for unit vectors.
...
Never write two subscripted variables separated by / inside a single $...$. Always use 
rac{}{} instead.
- ? Correct: $\\gamma = 
rac{C_p}{C_v}$, $
rac{V_1}{V_2}$, $
rac{T_1}{T_2}$, $
rac{\lambda_e}{\lambda_p}$
- ? Wrong: $C_p/C_v$, $V_1/V_2$, $T_1/T_2$ ? MathJax sees chained subscripts and throws a parse error
...
- Ion charges MUST use 	ext{} for the sign character ? MathJax treats + and - inside ^{...} as binary operators, which causes parse errors. The only safe fix is to wrap the sign in 	ext{}:
...
- Always 
rac{numerator}{denominator} ? both arguments must be brace-wrapped even for single chars: 
rac{1}{2} not 
rac12.
...
5. ? Every 
rac, \sqrt, \sum, \int has all arguments in {}.
6. ? All ion charges use 	ext{} for the sign: ^{2	ext{+}}, ^{	ext{-}}, ^{	ext{+}} ? never raw ^{2+} or ^{-} or ^+.
7. ? No subscripted variable / subscripted variable inline ? always use 
rac{}{} for ratios of subscripted quantities.
